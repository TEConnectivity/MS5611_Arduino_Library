...
enum ms5611_status_code {
  ms5611_STATUS_OK = 0,
  ms5611_STATUS_ERR_OVERFLOW = 1,
  ms5611_STATUS_ERR_TIMEOUT = 4
};
...
public:
  ...
  *
  */
  void set_resolution(enum ms5611_resolution_osr res);
  ...
  *       - ms5611_status_i2c_transfer_error : Problem with i2c transfer
  *       - ms5611_status_no_i2c_acknowledge : I2C did not acknowledge
  *       - ms5611_status_crc_error : CRC check error on on the PROM
  * coefficients
  */
  enum ms5611_status read_temperature_and_pressure(float *temperature,
                                                   float *pressure);

private:
  enum ms5611_status write_command(uint8_t cmd);
  enum ms5611_status read_eeprom_coeff(uint8_t command, uint16_t *coeff);
  boolean crc_check(uint16_t *n_prom, uint8_t crc);
  enum ms5611_status conversion_and_read_adc(uint8_t cmd, uint32_t *adc);
  enum ms5611_status read_eeprom(void);

  uint16_t eeprom_coeff[MS5611_COEFFICIENT_NUMBERS];
  bool coeff_read = false;
  enum ms5611_status ms5611_write_command(uint8_t);
  enum ms5611_status ms5611_read_eeprom_coeff(uint8_t, uint16_t *);
  enum ms5611_status ms5611_read_eeprom(void);
  enum ms5611_status ms5611_conversion_and_read_adc(uint8_t, uint32_t *);

  enum ms5611_resolution_osr ms5611_resolution_osr;
  uint32_t conversion_time[6] = {
      MS5611_CONVERSION_TIME_OSR_256,  MS5611_CONVERSION_TIME_OSR_512,
      MS5611_CONVERSION_TIME_OSR_1024, MS5611_CONVERSION_TIME_OSR_2048,
